...
public class STFT {
  ...
  public double secondMaxAmpFreq = Double.NaN, secondMaxAmpDB = Double.NaN;
...
//    Arrays.sort(spectrumAmpOutDB);
// reverse the array
//    for(int i=0;i<spectrumAmpOutDB.length;i++) {
      // swap the elements
//      double temp = spectrumAmpOutDB[i];
//      spectrumAmpOutDB[i] = spectrumAmpOutDB[spectrumAmpOutDB.length-(i+1)];
//      spectrumAmpOutDB[spectrumAmpOutDB.length-(i+1)] = temp;

//    }

//    secondMaxAmpDB  = 20 * Math.log10(0.125/32768);
//    secondMaxAmpFreq = 0;
    ...
    double [] copy = new double[spectrumAmpOutDB.length];
    System.arraycopy( spectrumAmpOutDB, 0, copy, 0, spectrumAmpOutDB.length );
    int z = (int) maxAmpFreq;
    copy[z] = -100.0;

    secondMaxAmpDB  = 20 * Math.log10(0.125/32768);
    secondMaxAmpFreq = 0;

    for (int j = 1; j < copy.length; j++) {  // skip the direct current term
      if (copy[j] > secondMaxAmpDB) {
        secondMaxAmpDB  = copy[j];
        secondMaxAmpFreq = j;
      }
    }

//    int z = (int) maxAmpFreq;
//    spectrumAmpOutDB[z] = -100.0;

//    String maxAmpDB1 = String.valueOf(maxAmpDB);
//    List<String> list = new ArrayList<String>(Arrays.asList(spectrumAmpOutDB));
//    list.removeAll(Arrays.asList("a"));
//    array = list.toArray(EMPTY_STRING_ARRAY);

//    spectrumAmpOutDB = ArrayUtils.removeElement(spectrumAmpOutDB, maxAmpDB)



//    for (int j = 1; j < spectrumAmpOutDB.length; j++) {  // skip the direct current term
//      if (spectrumAmpOutDB[j] > secondMaxAmpDB) {
//        secondMaxAmpDB  = spectrumAmpOutDB[j];
//        secondMaxAmpFreq = j;
//      }
//    }
    ...
    secondMaxAmpFreq = secondMaxAmpFreq * sampleRate / fftLen;
